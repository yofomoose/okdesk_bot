...
**📝 Как создать заявку:**
1. Используйте команду `/issue` или кнопку в меню
2. Опишите проблему одним сообщением
3. Получите номер заявки и ссылку на портал

**💡 Примеры заявок:**
• "Не работает принтер HP в кабинете 205"
• "Нужен доступ к корпоративной сети"
• "Сломался монитор Dell, мерцает экран"
• "Не запускается программа 1С Предприятие"
...
**📞 Поддержка:**
Все заявки автоматически поступают специалистам.
Среднее время реакции - 15 минут в рабочее время.
"""
...
async def create_issue_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        "💡 **Пример:**\n"
        "`Не работает принтер HP LaserJet в кабинете 205, горит красная лампочка`\n\n"
        "📎 Вы также сможете прикрепить фото или документы после создания заявки.",
        parse_mode="Markdown"
    )
    await state.set_state(IssueStates.waiting_for_description)
    ...
    try:
        ...
        if response and "id" in response:
            ...
            sent_message = await message.answer(
                ...
                f"📝 **Заголовок:** {title}\n"
                f"📅 **Дата создания:** {issue.created_at.strftime('%d.%m.%Y %H:%M')}\n\n"
                f"💡 Нажмите кнопку ниже, чтобы открыть заявку в портале\n"
                f"🔐 Вход будет выполнен автоматически",
                reply_markup=keyboard,
                parse_mode="Markdown"
            )
...
async def show_my_issues(callback: CallbackQuery):
    ...
    if not issues:
        await callback.message.edit_text(
            ...
            "Опишите проблему, и наши специалисты помогут вам.",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="📝 Создать первую заявку", callback_data="create_issue")],
                [InlineKeyboardButton(text="🏠 Главное меню", callback_data="main_menu")]
            ]),
            parse_mode="Markdown"
        )
...
async def show_issues_list(callback: CallbackQuery, open_issues, closed_issues, list_type):
    ...
    if not issues:
        ...
        for i, issue in enumerate(issues, 1):
            ...
            created_date = issue.created_at.strftime('%d.%m')
            text += f"{i}. `#{issue.issue_number}` • {issue.title}\n"
            text += f"   {status_emoji} • {created_date}\n\n"
    ...
    for i, issue in enumerate(issues[:10], 1):
        ...
        keyboard_buttons.append([
            InlineKeyboardButton(
                text=f"{i}. #{issue.issue_number} • {title_short}",
...
async def handle_rating(callback: CallbackQuery):
    ...
    try:
        ...
        try:
            ...
            # Проверяем, не была ли заявка уже оценена
            if issue.rating is not None:
                await callback.answer(f"ℹ️ Заявка уже была оценена: {'⭐' * issue.rating} ({issue.rating}/5)")
                return
            ...
            try:
                ...
                # Определяем текст благодарности в зависимости от оценки
                if rating >= 4:
                    thanks_text = "Мы рады, что смогли помочь вам! 😊"
                elif rating == 3:
                    thanks_text = "Спасибо за честную оценку! Мы будем работать над улучшением сервиса. 🤝"
                else:
                    thanks_text = "Спасибо за обратную связь. Мы обязательно учтем ваши замечания и улучшим качество обслуживания. 🙏"
                ...
                thank_message = (
                    ...
                    f"💬 Оценка сохранена в комментариях к заявке.\n\n"
                    f"_{thanks_text}_"
                )
...
async def show_profile(callback: CallbackQuery):
    ...
    if user.user_type == "physical":
        ...
        profile_text = (
            ...
            f"🔢 **ИНН:** {user.inn_company or 'Не указан'}\n"
            f"📅 **Дата регистрации:** {user.created_at.strftime('%d.%m.%Y')}"
        )
    else:
